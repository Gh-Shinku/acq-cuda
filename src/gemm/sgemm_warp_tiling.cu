...
template <typename T, int BLOCK_TILE_SIZE_X, int BLOCK_TILE_SIZE_Y,
          int BLOCK_TILE_SIZE_K, int WARP_TILE_SIZE_X, int WARP_TILE_SIZE_Y,
          int THREAD_TILE_SIZE_X, int THREAD_TILE_SIZE_Y>
__global__ void sgemm_warp_tiling(int m, int n, int k, T const* A,
  ...
  static_assert(WARP_TILE_SIZE_X % THREAD_TILE_SIZE_X == 0);
  static_assert(WARP_TILE_SIZE_Y % THREAD_TILE_SIZE_Y == 0);

  constexpr int WARP_SIZE{32};
  constexpr int NUM_WARP_TILES_X{BLOCK_TILE_SIZE_X / WARP_TILE_SIZE_X};
  constexpr int NUM_WARP_TILES_Y{BLOCK_TILE_SIZE_Y / WARP_TILE_SIZE_Y};
  constexpr int NUM_WARPS_PER_BLOCK{NUM_WARP_TILES_X * NUM_WARP_TILES_Y};
  constexpr int NUM_THREAD_TILES_X{WARP_TILE_SIZE_X / THREAD_TILE_SIZE_X};
  constexpr int NUM_THREAD_TILES_Y{WARP_TILE_SIZE_Y / THREAD_TILE_SIZE_Y};
  constexpr int NUM_THREADS_PER_BLOCK{NUM_WARPS_PER_BLOCK * WARP_SIZE};
  static_assert(NUM_THREAD_TILES_X * NUM_THREAD_TILES_Y == WARP_SIZE);
  ...
  int const lane_idx{thread_linear_idx % WARP_SIZE};

  int const warp_tile_x{warp_idx % NUM_WARP_TILES_X};
  int const warp_tile_y{warp_idx / NUM_WARP_TILES_X};
  int const lane_tile_x{lane_idx % NUM_THREAD_TILES_X};
  int const lane_tile_y{lane_idx / NUM_THREAD_TILES_X};

  int const thread_tile_row_base{warp_tile_y * WARP_TILE_SIZE_Y +
                                 lane_tile_y * THREAD_TILE_SIZE_Y};
  int const thread_tile_col_base{warp_tile_x * WARP_TILE_SIZE_X +
                                 lane_tile_x * THREAD_TILE_SIZE_X};

  __shared__ T A_thread_block_tile[BLOCK_TILE_SIZE_Y][BLOCK_TILE_SIZE_K + 1];
  __shared__ T B_thread_block_tile[BLOCK_TILE_SIZE_K][BLOCK_TILE_SIZE_X + 1];
...
#pragma unroll
      for (int thread_tile_row_idx{}; thread_tile_row_idx < THREAD_TILE_SIZE_Y;
        ...
        A_thread_regs[thread_tile_row_idx] =
            A_thread_block_tile[A_thread_block_tile_row_idx][k_i];
      }
...
#pragma unroll
      for (int thread_tile_col_idx{}; thread_tile_col_idx < THREAD_TILE_SIZE_X;
        ...
        B_thread_regs[thread_tile_col_idx] =
            B_thread_block_tile[k_i][B_thread_block_tile_col_idx];
      }
...
template <typename T, int BLOCK_TILE_SIZE_X, int BLOCK_TILE_SIZE_Y,
          int BLOCK_TILE_SIZE_K, int WARP_TILE_SIZE_X, int WARP_TILE_SIZE_Y,
          int THREAD_TILE_SIZE_X, int THREAD_TILE_SIZE_Y>
void launch_sgemm_warp_tiling_variant(int m, int n, int k, T const* A,
                                      T const* B, T const* C, T* D, T alpha,
                                      T beta, cudaStream_t stream) {
  constexpr int WARP_SIZE{32};
  constexpr int NUM_WARPS_PER_BLOCK{BLOCK_TILE_SIZE_X / WARP_TILE_SIZE_X *
                                    BLOCK_TILE_SIZE_Y / WARP_TILE_SIZE_Y};
  constexpr int NUM_THREADS_PER_BLOCK{NUM_WARPS_PER_BLOCK * WARP_SIZE};
  ...
  sgemm_warp_tiling<T, BLOCK_TILE_SIZE_X, BLOCK_TILE_SIZE_Y,
...
template <typename T>
void launch_sgemm_warp_tiling(int m, int n, int k, T const* A, T const* B,
  ...
  if (max_mn <= 512) {
    ...
    launch_sgemm_warp_tiling_variant<T, 64, 64, 8, 32, 16, 4, 4>(
...
void launch_sgemm_warp_tiling(const SgemmProblem& problem, const float* a,
                              const float* b, const float* c, float* d,
                              cudaStream_t stream) {
  launch_sgemm_warp_tiling<float>(problem.m, problem.n, problem.k, a, b, c, d,
